...
def test_research_depth_sets_both_rounds_without_env(monkeypatch):
    for var in ("FIXEDINCOMEAGENT_MAX_DEBATE_ROUNDS", "FIXEDINCOMEAGENT_MAX_RISK_ROUNDS"):
...
def test_env_round_counts_win_over_selection(monkeypatch):
    monkeypatch.setenv("FIXEDINCOMEAGENT_MAX_DEBATE_ROUNDS", "2")
    monkeypatch.setenv("FIXEDINCOMEAGENT_MAX_RISK_ROUNDS", "4")
...
def test_partial_env_only_overrides_that_count(monkeypatch):
    monkeypatch.setenv("FIXEDINCOMEAGENT_MAX_DEBATE_ROUNDS", "2")
    monkeypatch.delenv("FIXEDINCOMEAGENT_MAX_RISK_ROUNDS", raising=False)
