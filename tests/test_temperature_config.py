...
class TestTemperatureEnvOverlay:
    def test_env_sets_temperature(self, monkeypatch):
        import fixedincomeagent.default_config as dc
        monkeypatch.setenv("FIXEDINCOMEAGENT_TEMPERATURE", "0.2")
        importlib.reload(dc)
        ...
        monkeypatch.delenv("FIXEDINCOMEAGENT_TEMPERATURE", raising=False)
        importlib.reload(dc)
