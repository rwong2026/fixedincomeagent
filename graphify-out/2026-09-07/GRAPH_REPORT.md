...
## Corpus Check
- 231 files · ~285,254 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3056 nodes · 6384 edges · 176 communities (139 shown, 33 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 284 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e544ff00`
...
- `test_tool_nodes_unfiltered_by_default()` --uses--> `TradingAgentsGraph`  [INFERRED]
  tests/test_ablation.py → fixedincomeagent/graph/trading_graph.py
- `_bare_graph()` --uses--> `TradingAgentsGraph`  [INFERRED]
  tests/test_checkpoint_lifecycle.py → fixedincomeagent/graph/trading_graph.py
- `TestCheckpointSignature` --uses--> `TradingAgentsGraph`  [INFERRED]
  tests/test_checkpoint_resume.py → fixedincomeagent/graph/trading_graph.py
...
### Community 2 - "test_llm_max_tokens.py"
Cohesion: 0.06
Nodes (47): AsyncCallbackManagerForLLMRun, BaseLLMClient, BaseMessage, CallbackManagerForLLMRun, ChatGoogleGenerativeAI, ChatResult, _coerce_max_tokens(), Validate a ``max_tokens`` value to a positive int (env vars are strings). (+39 more)
...
Cohesion: 0.08
Nodes (23): ABC, AnthropicClient, Client for Anthropic Claude models., Validate model for Anthropic., AzureOpenAIClient, Client for Azure OpenAI deployments. Requires environment variables:…, Azure accepts any deployed model name., BaseLLMClient (+15 more)
...
Cohesion: 0.17
Nodes (28): callable, forwards_implied_baseline(), _implied_change_bp(), Maturity in years of a configured tenor label like ``"10Y"``., Front-of-curve forward minus spot, in bp (see module docs)., Forward-curve-implied moves over the horizon (see module docstring)., Re-skin baseline predictions as BacktestRuns scored by score_backtest. Realized…, _tenor_years() (+20 more)

### Community 8 - "test_fi_consistency.py"
Cohesion: 0.13
Nodes (42): _annotate(), check_consistency(), ConsistencyWarning, fi_consistency_check_node(), _is_directional(), parse_trader_decision(), Fixed-Income Risk Consistency Check: pure-logic validator (no LLM). Sits…, Signed magnitude ordinal: up positive, down negative, neutral zero. (+34 more)
...
Cohesion: 0.12
Nodes (18): True when only fixed-income analysts are active., Main class that orchestrates the trading agents framework., TradingAgentsGraph, unit, The market analyst is bound (and prompt-instructed) to call…, test_market_toolnode_can_execute_verified_snapshot(), test_tool_nodes_have_handle_tool_errors_enabled(), Symbol normalization must apply on every yfinance path, not just price fetch.… (+10 more)

### Community 12 - "TestDeferredReflection"
Cohesion: 0.05
Nodes (23): Pick the benchmark ticker for alpha calculation against ``ticker``.…, Fetch raw and alpha return for ticker over holding_days from trade_date.…, Resolve pending log entries for ticker at the start of a new run. Fetches…, Resolve pending log entries for fixed-income runs using Treasury curve…, Only the matching entry is modified; all other entries remain unchanged., A pre-existing .tmp file is overwritten; the log is correctly updated., All fields intact and blank line between tag and DECISION preserved after…, Return figures are present in the human message sent to the LLM. (+15 more)
...
Cohesion: 0.13
Nodes (23): create_fundamentals_analyst(), create_market_analyst(), create_news_analyst(), create_bear_researcher(), create_bull_researcher(), create_aggressive_debator(), create_conservative_debator(), create_neutral_debator() (+15 more)

### Community 16 - "_stub"
Cohesion: 0.07
Nodes (22): _cache_path(), CotFormatError, _fmt(), get_cot_data(), _load_rows(), _parse_rows(), date, ValueError (+14 more)

### Community 17 - "Reflector"
Cohesion: 0.11
Nodes (20): Any, BaseModel, Argument tagging scaffold for credit assignment (Phase 6, Task 6.1).…, A debate argument tagged for credit assignment (scaffolding only)., Wrapper so with_structured_output can return a list of tags., Tag individual debate arguments via one structured LLM call. Returns an empty…, tag_debate_arguments(), TaggedArgument (+12 more)

### Community 18 - "cli/main.py"
Cohesion: 0.09
Nodes (30): CLI Welcome Screen, display_announcements(), fetch_announcements(), Fetch announcements from endpoint. Returns dict with announcements and settings., Display announcements panel. Prompts for Enter if require_attention is True., analyze(), classify_message_type(), create_layout() (+22 more)

### Community 19 - "create_portfolio_manager"
Cohesion: 0.10
Nodes (21): create_portfolio_manager(), Portfolio Manager: synthesises the risk-analyst debate into the final decision.…, PortfolioDecision, Structured output produced by the Portfolio Manager. The model fills every…, Render a PortfolioDecision back to the markdown shape the rest of the system…, render_pm_decision(), main(), _make_pm_state() (+13 more)

### Community 20 - "AgentState"
Cohesion: 0.11
Nodes (19): AgentState, DirectionDebateState, InvestDebateState, TypedDict, State for the yield-direction debate (higher vs lower yields)., State for the curve-shape debate (steepener vs flattener). Runs second,…, RiskDebateState, ShapeDebateState (+11 more)

### Community 21 - "test_macro_policy_analyst.py"
Cohesion: 0.15
Nodes (25): create_macro_policy_analyst(), InflationComponentTrajectory, MacroPolicyReport, MarketImpliedExpectations, Trajectory assessment for a single inflation component., Market-based inflation expectations from TIPS breakevens., Survey-based inflation expectations., Component-level inflation and macro analysis. CRITICAL: Do NOT average these… (+17 more)

### Community 22 - "DirectionCall"
Cohesion: 0.13
Nodes (26): DirectionCall, Structured yield-direction prediction for a single tenor., Render a DirectionCall to markdown., Structured curve-shape prediction for a single spread., Render a ShapeCall to markdown., Render a DirectionOutlook to markdown for the trader's prompt context., render_direction_call(), render_direction_outlook() (+18 more)

### Community 23 - "test_backtest_scoring.py"
Cohesion: 0.15
Nodes (32): Backtesting harness: point-in-time replay of the FI pipeline., BacktestRun, One replayed test date: predicted calls vs. realized outcomes., BacktestScorecard, _bucketize_change(), _cal_bin_index(), CalibrationBin, _rate() (+24 more)
...
Cohesion: 0.12
Nodes (14): crypto_base(), is_yahoo_safe(), _normalize_crypto(), normalize_symbol(), Symbol normalization and market-data error types for vendor calls. Yahoo…, Map a user/broker symbol to its canonical Yahoo Finance symbol. Resolution…, True when ``symbol`` only contains characters Yahoo symbols use., Return the crypto base (e.g. ``BTC``) for a known USD/USDT/USDC-quoted crypto… (+6 more)

### Community 27 - "openai_client.py"
Cohesion: 0.10
Nodes (21): _is_native_openai_base_url(), OpenAIClient, ProviderSpec, Any, Whether the (native OpenAI) model accepts ``reasoning_effort``., Declarative config for one OpenAI-compatible provider. The OpenAI-compatible…, True when ``base_url`` is unset or points at api.openai.com. The Responses API…, Client for OpenAI, Ollama, OpenRouter, and xAI providers. For native OpenAI… (+13 more)
...
Cohesion: 0.12
Nodes (20): NoMarketDataError, A vendor returned no usable rows for a symbol (empty result or stale data).…, Execute a yfinance call with exponential backoff on rate limits. yfinance…, StockstatsUtils, yf_retry(), get_balance_sheet(), get_cashflow(), get_fundamentals() (+12 more)
...
### Community 39 - "market_data_validator.py"
Cohesion: 0.12
...
Cohesion: 0.18
Nodes (15): ALFRED (Archival FRED) point-in-time vintage fetcher. Returns economic data as…, _fred_today(), get_api_key(), get_macro_data(), _get_session(), Session, FRED (Federal Reserve Economic Data) macro vendor. Fetches macroeconomic time…, Map a friendly alias to a FRED series ID, or pass a raw ID through. Raises… (+7 more)
...
Cohesion: 0.21
Nodes (26): ablation_disabled_tools(), AblationConfig, Config for ablation testing., Map ablation flags to the tool names to drop. Ablation removes DATA, not agents…, _bound_names(), Ablation support (Task 7.5): run the pipeline with data sources removed.…, _state(), test_ablation_config_defaults() (+18 more)

### Community 43 - "TradingMemoryLog"
Cohesion: 0.10
Nodes (11): Append-only markdown log of trading decisions and reflections., Replace pending tag and append REFLECTION section using atomic write. Finds the…, Apply multiple outcome updates in a single read + atomic write. Each element of…, Build a resolved entry tag, recording the outcome's known-by date.…, Drop oldest resolved blocks when their count exceeds max_entries. Pending…, Parse all entries from log. Returns list of dicts., Return entries with outcome:pending (for Phase B)., Return formatted past context string for agent prompt injection. When ``as_of``… (+3 more)
...
Cohesion: 0.14
Nodes (22): calculate_treasury_curve_benchmark(), evaluate_directional_hit(), _fetch_tenor_observations(), parse_direction_calls(), Any, Treasury curve equal-weight benchmark calculation for Fixed Income reflection.…, Calculate Treasury curve benchmark yield changes and decision hit-rate. Args:…, Extract normalized per-tenor direction calls from markdown or a dictionary. (+14 more)
...
### Community 49 - "test_fi_graph_setup.py"
Cohesion: 0.07
Nodes (40): _configured_spread_text(), create_curve_technicals_analyst(), Describe the configured spread set so the prompt tracks config., create_macro_calendar_analyst(), AnalystExecutionPlan, AnalystNodeSpec, AnalystWallTimeTracker, build_analyst_execution_plan() (+32 more)

### Community 50 - "test_fi_portfolio_manager.py"
Cohesion: 0.15
Nodes (19): GraphSetup, Handles the setup and configuration of the agent graph., _decision(), _free_text_llm(), _graph(), RunnableLambda, unit, Fixed-Income Portfolio Manager (Task 5.3) and the completed FI graph wiring.… (+11 more)
...
### Community 62 - "NormalizedChatOpenAI"
Cohesion: 0.16
Nodes (13): ChatOpenAI, is_openai_compatible(), LocalCompatibleChatOpenAI, NormalizedChatOpenAI, ChatOpenAI with normalized content output and capability-aware binding. The…, Whether ``provider`` is served by the OpenAI-compatible registry., OpenAI-compatible client for arbitrary local servers (LM Studio, vLLM,…, parametrize (+5 more)

### Community 63 - "schemas.py"
Cohesion: 0.12
Nodes (34): create_direction_research_manager(), Direction Research Manager: judges the higher/lower yields debate into a per-…, create_fi_portfolio_manager(), Fixed-Income Portfolio Manager: final synthesis of the desk's curve calls.…, create_shape_research_manager(), Shape Research Manager: judges the steepener/flattener debate into a per-spread…, DirectionOutlook, BaseModel (+26 more)
...
### Community 67 - "set_config"
Cohesion: 0.19
Nodes (11): Update the configuration with custom values. Dict-valued keys (e.g.…, set_config(), fixture, _tmp_cache(), _no_data(), unit, _raises(), Vendor router must respect the configured chain and never silently hide a… (+3 more)
...
### Community 69 - "test_debate_opening.py"
Cohesion: 0.36
Nodes (11): _capturing_llm(), _investment_state(), parametrize, unit, The first speaker in each debate must not rebut a nonexistent argument (#1176).…, _risk_state(), test_helper_marks_empty_and_passes_through(), test_researcher_opening_has_no_phantom_opponent() (+3 more)
...
Cohesion: 0.20
Nodes (19): ConditionalLogic, Determine if risk analysis should continue., Determine if sentiment-analyst tool round should continue. Method name keeps…, Handles conditional logic for determining graph flow., Determine if debate should continue., _direction_state(), unit, Dual-track FI debate routing (Task 4.4). `should_continue_direction_debate`… (+11 more)
...
### Community 77 - "test_structured_agents.py"
Cohesion: 0.20
Nodes (14): create_research_manager(), Research Manager: turns the bull/bear debate into a structured investment plan…, Render a ResearchPlan to markdown for storage and the trader's prompt context., Structured investment plan produced by the Research Manager. Hand-off to the…, render_research_plan(), ResearchPlan, _make_rm_state(), unit (+6 more)
...
Cohesion: 0.11
Nodes (13): _price_df(), Tests for TradingMemoryLog — storage, deferred reflection, PM injection, legacy…, No rotation when resolved count <= max_entries., Only 1 data point available → returns all-None, no crash., Store a decision then immediately resolve it via the API., Minimal DataFrame matching yfinance .history() output shape. Uses a…, Same-ticker entries in same-ticker section; cross-ticker entries in cross-…, Cross-ticker entries show only the REFLECTION text, not the full DECISION. (+5 more)

### Community 83 - "_stub"
Cohesion: 0.08
Nodes (21): _cache_path(), get_supply_chain_pressure(), GscpiFormatError, _load_rows(), _parse_rows(), date, ValueError, NY Fed Global Supply Chain Pressure Index (GSCPI): monthly index. Source… (+13 more)
...
Cohesion: 0.34
Nodes (13): create_flattener_researcher(), create_steepener_researcher(), Wire the FI dual-track debate after the analyst chain. Higher Yields <-> Lower…, _capturing_llm(), unit, Shape debate researchers: steepener vs flattener (Task 4.2). Run after the…, _shape_state(), test_flattener_opening_has_no_phantom_opponent() (+5 more)
...
### Community 91 - "_CapturingStubLLM"
Cohesion: 0.33
Nodes (3): RunnableLambda, _CapturingStubLLM, Offline LLM recording every bind_tools list it receives.
...
Cohesion: 0.09
Nodes (21): get_config(), initialize_config(), Initialize the configuration with default values., Get the current configuration., _fmt_meeting(), get_fomc_calendar(), Meeting, date (+13 more)

### Community 95 - "test_news_analyst_prompt.py"
Cohesion: 0.33
Nodes (5): Guard the news analyst prompt against tool-signature drift (#1116). The prompt…, The prompt must instruct the LLM to distinguish cut/hold/hike., test_get_news_takes_ticker_not_query(), test_news_prompt_decomposes_rate_action_probabilities(), test_news_prompt_matches_get_news_signature()
...
Cohesion: 0.40
Nodes (5): FredNotConfiguredError, Raised when FRED is selected but no API key is configured. A…, If FRED is not configured, fail open with None and do not crash., test_calculate_treasury_curve_benchmark_missing_api_key(), VendorNotConfiguredError
...
Cohesion: 0.15
Nodes (20): date, _fallback(), _FeedShapeError, get_fed_speeches(), _parse_items(), _parse_json_items(), Fed speeches vendor: recent speeches by Federal Reserve officials. Source: the…, Fetch recent Fed official speeches as a markdown report. Tries the RSS feed… (+12 more)
...
### Community 105 - "._create_tool_nodes"
Cohesion: 0.50
Nodes (3): Create tool nodes for different data sources using abstract methods.…, test_tool_nodes_unfiltered_by_default(), ToolNode

### Community 106 - "test_cli_fi_reskin.py"
Cohesion: 0.27
Nodes (11): Update analyst statuses based on accumulated report state. Logic: - Store new…, update_analyst_statuses(), unit, Tests for FI CLI re-skinning and dual-track support in CLI., test_analyst_order_defaults_to_fi_first(), test_analyst_type_enum_has_fi_analysts(), test_get_ticker_defaults_to_ust(), test_message_buffer_equity_mode() (+3 more)

### Community 108 - "MessageBuffer"
Cohesion: 0.16
Nodes (4): MessageBuffer, Initialize agent status and report sections based on selected analysts. Args:…, Count reports that are finalized (their finalizing agent is completed). A…, Whether the selected analyst set triggers the FI graph track.
...
Cohesion: 0.13
Nodes (18): get_stock_data(), tool, Retrieve stock price data (OHLCV) for a given ticker symbol. Uses the…, get_macro_indicators(), tool, Retrieve a macroeconomic indicator time series from FRED (Federal Reserve…, get_prediction_markets(), tool (+10 more)

### Community 113 - "sentiment_analyst.py"
Cohesion: 0.12
Nodes (17): _build_system_message(), create_sentiment_analyst(), create_social_media_analyst(), Sentiment analyst — multi-source sentiment analysis for a target ticker.…, Assemble the sentiment-analyst system message with structured data blocks., Deprecated alias for :func:`create_sentiment_analyst`. Kept so existing code…, Create a sentiment analyst node for the trading graph. Pre-fetches news +…, Backwards-compatibility shim for the renamed module. The agent is now… (+9 more)

### Community 114 - "._get_provider_kwargs"
Cohesion: 0.40
Nodes (4): Any, _clean_identity_value(), Return a trimmed string, or None for empty / placeholder-ish values., Get provider-specific kwargs for LLM client creation.
...
### Community 123 - "test_no_data_handling.py"
Cohesion: 0.22
Nodes (4): unit, Tests that empty vendor results never become fabricated data. Covers two…, TestLoadOhlcvNoPoison, TestRouteToVendorSentinel
...
Cohesion: 0.12
Nodes (9): Run the trading agents graph for a company on a specific date. ``asset_type``…, Restore the plain uncheckpointed graph after a checkpointed run., Context-manager form of begin/end_checkpoint for the propagate path., FinancialSituationMemory must not be importable from the memory module., rank_bm25 must not be present in the memory module namespace., TradingAgentsGraph must not expose reflect_and_remember., create_portfolio_manager accepts only llm; passing memory= raises TypeError., propagate() completes and stores the decision after the redesign. (+1 more)
...
Cohesion: 0.18
Nodes (12): field_validator, _coerce_optional_float(), Structured transaction proposal produced by the Trader. The trader reads the…, Render a TraderProposal to markdown. The trailing ``FINAL TRANSACTION PROPOSAL:…, render_trader_proposal(), TraderProposal, create_trader(), Trader: turns the Research Manager's investment plan into a concrete… (+4 more)

### Community 132 - "dataflows/utils.py"
Cohesion: 0.29
Nodes (3): DataFrame, save_output(), SavePathType

### Community 133 - "NormalizedAzureChatOpenAI"
Cohesion: 0.29
Nodes (5): AzureChatOpenAI, NormalizedAzureChatOpenAI, Any, AzureChatOpenAI with normalized content output., Return configured AzureChatOpenAI instance.
...
### Community 174 - "get_YFin_data_online"
Cohesion: 0.40
Nodes (5): get_YFin_data_online(), unit, yfinance treats ``end`` as exclusive; we must request one extra day so the…, test_get_yfin_requests_inclusive_end(), test_load_ohlcv_requests_inclusive_end()
...
### Community 177 - "Propagator"
Cohesion: 0.17
Nodes (6): Propagator, Any, Get arguments for the graph invocation. Args: callbacks: Optional list of…, Handles state initialization and propagation through the graph., Initialize with configuration parameters., Initialize the trading agents graph and components. Args: selected_analysts:…
...
- **Why does `TradingAgentsGraph` connect `TradingAgentsGraph` to `TestLegacyRemoval`, `test_llm_max_tokens.py`, `test_backtest_runner.py`, `test_equity_state_logging`, `TestDeferredReflection`, `AgentState`, `build_instrument_context`, `FixedIncomeAnalysis`, `SignalProcessor`, `test_checkpoint_resume.py`, `test_fi_graph_wiring.py`, `test_ablation.py`, `Propagator`, `test_fi_graph_setup.py`, `test_fi_portfolio_manager.py`, `._run_graph`, `test_llm_max_retries.py`, `test_instrument_identity.py`, `TestProviderKwargsTemperature`, `test_memory_log.py`, `thread_id`, `checkpointer.py`, `test_checkpoint_lifecycle.py`, `._create_tool_nodes`, `._get_provider_kwargs`, `test_reporting.py`, `test_memory_pointintime.py`?**
  _High betweenness centrality (0.153) - this node is a cross-community bridge._
- **Why does `get_config()` connect `get_config` to `test_backtest_runner.py`, `test_baselines.py`, `stockstats_utils.py`, `agents/__init__.py`, `_stub`, `get_curve_spreads`, `DirectionCall`, `_stub`, `ism_prices_paid.py`, `consumer_inflation_expectations.py`, `test_fi_graph_setup.py`, `in_window`, `schemas.py`, `shelter_rents.py`, `_stub`, `test_fi_trader.py`, `used_vehicle_index.py`, `interface.py`, `.__init__`?**
  _High betweenness centrality (0.068) - this node is a cross-community bridge._
- **Why does `set_config()` connect `set_config` to `_make_api_request`, `_stub`, `test_polymarket.py`, `_request_stub`, `test_baselines.py`, `_assert_ohlcv_not_stale`, `get_YFin_data_online`, `agents/__init__.py`, `_stub`, `Propagator`, `FedSpeechesTests`, `_stub`, `test_consumer_expectations.py`, `_stub`, `._write_csv`, `test_no_data_handling.py`, `get_config`, `._write_csv`?**
