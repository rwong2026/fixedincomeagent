...
def get_user_selections():
    ...
    # Step 3: Output language (skipped when set via FIXEDINCOMEAGENT_OUTPUT_LANGUAGE)
    if os.environ.get("FIXEDINCOMEAGENT_OUTPUT_LANGUAGE"):
    ...
    # supplied through FIXEDINCOMEAGENT_MAX_DEBATE_ROUNDS / _MAX_RISK_ROUNDS we keep
    # the run non-interactive and honor the env values (#977).
    depth_from_env = bool(os.environ.get("FIXEDINCOMEAGENT_MAX_DEBATE_ROUNDS")) and bool(
        os.environ.get("FIXEDINCOMEAGENT_MAX_RISK_ROUNDS")
    )
    if depth_from_env:
    ...
    # Step 6: LLM Provider (skipped when set via FIXEDINCOMEAGENT_LLM_PROVIDER).
    # The backend URL comes from FIXEDINCOMEAGENT_LLM_BACKEND_URL when set,
    # otherwise the provider's default endpoint — the same value the menu
    # would have picked.
    provider_from_env = bool(os.environ.get("FIXEDINCOMEAGENT_LLM_PROVIDER"))
    ...
    if os.environ.get("FIXEDINCOMEAGENT_QUICK_THINK_LLM") or os.environ.get("FIXEDINCOMEAGENT_DEEP_THINK_LLM"):
    ...
    # settable via its FIXEDINCOMEAGENT_* env var; when that var is set (or the
...
def _build_run_config(selections: dict, checkpoint: bool | None) -> dict:
    ...
    # (FIXEDINCOMEAGENT_MAX_DEBATE_ROUNDS / _MAX_RISK_ROUNDS) wins over the
    # interactive selection — leave the env-applied value in place (#977).
    if not os.environ.get("FIXEDINCOMEAGENT_MAX_DEBATE_ROUNDS"):
        config["max_debate_rounds"] = selections["research_depth"]
    if not os.environ.get("FIXEDINCOMEAGENT_MAX_RISK_ROUNDS"):
    ...
    # the flag preserves FIXEDINCOMEAGENT_CHECKPOINT_ENABLED / the default (#976).
...
def run_analysis(checkpoint: bool | None = None):
    ...
    # Initialize the analysis orchestrator with callbacks bound to LLMs
    ...
    with Live(layout, refresh_per_second=4):
        ...
        # Stream the analysis using FixedIncomeAnalysis
        trace = []
        for chunk in analysis.run_streaming(
            label=selections["ticker"],
...
def analyze(
    checkpoint: bool | None = typer.Option(
        ...
        "crashed run can resume). Omit to honor FIXEDINCOMEAGENT_CHECKPOINT_ENABLED.",
    ),
    ...
    if clear_checkpoints:
        from fixedincomeagent.graph.checkpointer import clear_all_checkpoints
